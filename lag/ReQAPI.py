...
class APIClient:
    def __init__(self):
        ...
        detail_vers = bdversion() or {}
        self.is_emulator = False
        self.language = "en"
        self.base_url = detail_vers.get("server_url") or "https://loginbp.ggpolarbear.com/"
        self.client_version = detail_vers.get("remote_version") or "1.126.1"
        self.release_version = detail_vers.get("latest_release_version") or "OB54"
        ...
        print(f"[INIT] client_version={self.client_version} | release_version={self.release_version}")
    ...
    def auth_guest_token(self, uid, password):
        ...
        try:
            ...
            data = resp.json()
            print(f"[DEBUG] guest_token HTTP={resp.status_code} | uid={uid} | open_id={data.get('open_id')}")

            if "access_token" not in data:
                print(f"❌ guest_token FAIL: {data}")
    ...
    def MajorLogin(self):
        ...
        try:
            ...
            print(f"[DEBUG] MajorLogin POST → {url}")
            print(f"[DEBUG] MajorLogin version={self.release_version} | client={self.client_version}")
            print(f"[DEBUG] MajorLogin open_id={self._data.open_id} | token_len={len(str(self._data.access_token))}")
            ...
            print(f"[DEBUG] MajorLogin HTTP={response.status_code} | len={len(response.content)}B")
            if len(response.content) > 0:
                print(f"[DEBUG] MajorLogin hex[:60]={response.content[:60].hex()}")

            if response.status_code != 200:
                print(f"[DEBUG] MajorLogin body={response.text[:300]}")
                return

            pb = ProtoBuf(response.content)
            res = pb.protobuf()
            print(f"[DEBUG] MajorLogin parsed fields: {list(res.keys())}")
            ...
            print(f"[DEBUG] MajorLogin → account_id={self._data.account_id} | "
                  f"token={'OK' if self._data.login_token else 'MISSING'} | "
                  f"key={'OK' if self._data.key else 'MISSING'} | "
                  f"iv={'OK' if self._data.iv else 'MISSING'}")
        except Exception as e:
            print(f"❌ MajorLogin EXC: {e}")
            import traceback
            traceback.print_exc()
    ...
    def GetLoginData(self):
        try:
            ...
            print(f"[DEBUG] GetLoginData POST → {url}")

            response = self.session.post(
                url,
                headers={
                    "Authorization": "Bearer %s" % self._data.login_token,
                    "Host": self._data.base_url.replace("https://", "").replace("http://", "")
                }, data=AES_CBC128(
                    pb_encode(fields),
                    self.key, self.iv
                ),
            ...
            print(f"[DEBUG] GetLoginData HTTP={response.status_code} | len={len(response.content)}B")

            if response.status_code != 200:
                print(f"[DEBUG] GetLoginData body={response.text[:300]}")
                return
            ...
            if chat and len(chat) > 6:
                self._data.chat_port, self._data.chat_ip = chat[-5:], chat[:-6]
            if sv and len(sv) > 6:
                self._data.online_port, self._data.online_ip = sv[-5:], sv[:-6]

            print(f"[DEBUG] GetLoginData → online={self._data.online_ip}:{self._data.online_port} | "
                  f"chat={self._data.chat_ip}:{self._data.chat_port}")
        except Exception as e:
            print(f"❌ GetLoginData EXC: {e}")
            import traceback
            traceback.print_exc()
    ...
    def auth(self, access_token, is_emulator=False):
        try:
            ...
            if ":" in access_token:
                ...
                ok = self.auth_guest_token(int(uid), password)
                if ok:
                    return ok
            else:
                self.auth_token_inspect(access_token)

            print("[DEBUG] → Gọi MajorLogin...")
            self.MajorLogin()
            if not self._data.login_token:
                print("❌ MajorLogin FAIL (no login_token)")
                return "major_login failed"

            print("[DEBUG] → Gọi GetLoginData...")
            self.GetLoginData()
            if not getattr(self, "logindata", None):
                print("❌ GetLoginData FAIL")
                return "get_login_data failed"

            print("[DEBUG] → Build auth packet...")
            packet = self.TAO_PACKET_XT()
            if not packet:
                print("❌ TAO_PACKET_XT FAIL")
                return "auth_packet failed"

            return self._build_api_response(packet)
        except Exception as e:
            print(f"❌ auth EXC: {e}")
            import traceback
            traceback.print_exc()
