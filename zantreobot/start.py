...
<b>💾 RAM</b>
├ Đã dùng: <code>{memory_used_gb:.1f} GB</code>
├ Tổng: <code>{memory_total_gb:.1f} GB</code>
├ Đã dùng: <code>{memory_percent:.1f}%</code>
└ [{ram_bar}]

<b>⚡ CPU</b>
├ Đã dùng: <code>{cpu_percent:.1f}%</code>
├ Số nhân: <code>{cpu_count}</code>
└ [{cpu_bar}]

<b>💿 DISK</b>
├ Đã dùng: <code>{disk_used_gb:.1f} GB</code>
├ Tổng: <code>{disk_total_gb:.1f} GB</code>
├ Đã dùng: <code>{disk_percent:.1f}%</code>
└ [{disk_bar}]

<b>⏱️ UPTIME</b>
└ <code>{uptime_str}</code>

<b>🐧 HỆ ĐIỀU HÀNH</b>
├ <code>{platform.system()}</code>
└ <code>{platform.release()}</code>

<b>⛔ BLACKLIST</b>
└ <code>{len(blacklist)}</code> user

━━━━━━━━━━━━━━━━━━━━
<b>📩 Dùng /ping</b> để kiểm tra kết nối
</blockquote>
"""
        telegram_bot.reply_to(message, text, parse_mode="HTML")
...
def telegram_uptime(message):
    ...
    try:
        if not hasattr(telegram_bot, 'start_time'):
            telegram_bot.start_time = time.time()
        
        uptime_seconds = int(time.time() - telegram_bot.start_time)
        ...
        start_time_str = time.strftime('%H:%M:%S %d/%m/%Y', time.localtime(telegram_bot.start_time))
...
{str(days).zfill(2)} ngày  {str(hours).zfill(2)} giờ  {str(minutes).zfill(2)} phút  {str(seconds).zfill(2)} giây

[{bar}] {percent:.1f}%

🚀 Khởi động: <code>{start_time_str}</code>
