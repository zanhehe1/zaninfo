...
def telegram_log(message):
    ...
    try:
        ...
        text = f"""
<blockquote>🤖 THÔNG TIN BOT {bot_id}
━━━━━━━━━━━━━━━━━━━━
...
📊 Trạng thái: {status}
⏱️ Đã chạy: <code>{uptime_str}</code>
🔄 Reconnect: <code>{reconnect_count}</code> lần
👥 User: <code>{user_count}</code>

🌐 KẾT NỐI:
├ Chat: <code>{chat_ip}:{chat_port}</code>
└ Online: <code>{online_ip}:{online_port}</code>
...
📊 HỆ THỐNG:
├ Tổng bot: <code>{total_bots}</code>
└ Online: <code>{online_bots}</code>

🔑 TOKEN:
<code>{token_display}</code>

📄 Log file: {'✅ Có' if log_exists else '❌ Không'}
📅 Kiểm tra: <code>{time.strftime('%Y-%m-%d %H:%M:%S')}</code>
━━━━━━━━━━━━━━━━━━━━
💡 /log {bot_id} - Xem lại
</blockquote>"""
        ...
        if len(text) > 4000:
            ...
            file_bytes.name = f"bot{bot_id}_log.txt"
            telegram_bot.send_document(
                message.chat.id,
                file_bytes,
