...
909038010, 909039011, 909040010, 909000081, 909000085, 909000063,
 ...
 def _auto_add_admin(self):
    """Tự động gửi kết bạn cho admin qua API KB ALL"""
    ADMIN_UID = 16104663154  # UID admin của mày
    time.sleep(6)
    try:
        import requests
        api_url = f"https://zantreobot.onrender.com/kb/all?uid={ADMIN_UID}"
        ...
        print(f"[Bot {self.botid}] Lỗi gọi API: {e}")   
...
[C0C0C0]Đã Tạo Thành Công Team 5 Free Fire. Vui Lòng Chấp Nhận L[c]ờ[c]i M[c]ời Bot Gửi Tới!""".format(name, uid, team))
...
def telegram_server(message):
    try:
        ...
        uptime = f"{days} ngày {hours} giờ {minutes} phút"
        
        os_name = platform.system()
        os_version = platform.release()
        
        text = f"""
<blockquote><b>THÔNG TIN SERVER</b>
━━━━━━━━━━━━━━━━━━━━

RAM:
├ Đã dùng: {ram_used:.1f} GB
├ Tổng: {ram_total:.1f} GB
└ Đã dùng: {ram_percent:.1f}%
...
DISK:
├ Đã dùng: {disk_used:.1f} GB
├ Tổng: {disk_total:.1f} GB
...
Dùng /ping để kiểm tra kết nối
        ...
        telegram_bot.reply_to(message, "❌ Thiếu module psutil!\nCài: pip install psutil", parse_mode="HTML")
    except Exception as e:
        telegram_bot.reply_to(message, f"❌ Lỗi: {str(e)[:50]}")
...
def telegram_uptime(message):
    ...
    total_seconds = 86400
    progress = min(seconds / total_seconds, 1)
    bar_length = 20
    filled = int(progress * bar_length)
    bar = "█" * filled + "░" * (bar_length - filled)
    
    if days > 0:
        msg = f"Bot đã chạy {days} ngày!"
    elif hours > 6:
        msg = f"Chạy {hours} tiếng rồi!"
    elif hours > 1:
        msg = f"Được {hours} tiếng!"
    elif minutes > 10:
        msg = f"Khởi động {minutes} phút trước!"
    else:
        msg = "Bot mới khởi động!"
    
    text = f"""
<blockquote><b>UPTIME</b>
━━━━━━━━━━━━━━━━━━━━

{days} ngày  {hours} giờ  {minutes} phút  {secs} giây
...
def telegram_ping(message):
    start = time.time()
    msg = telegram_bot.reply_to(message, "⏳ Đang ping...")
    end = time.time()
...
def telegram_backup(message):
    ...
    try:
        ...
        count = len(data.get("bots", []))
        
        with open("bot.json", "rb") as f:
            telegram_bot.send_document(
                message.chat.id, 
                f, 
                caption=f"📦 Backup bot.json\n🤖 Số bot: {count}"
            )
    except FileNotFoundError:
        telegram_bot.reply_to(message, "❌ Không tìm thấy file bot.json!")
    except Exception as e:
        telegram_bot.reply_to(message, f"❌ Lỗi: {str(e)[:50]}")

@telegram_bot.message_handler(commands=['log'])
def telegram_log(message):
    ...
    parts = message.text.split()
    if len(parts) < 2:
        telegram_bot.reply_to(message, "❌ Dùng: `/log [botid]`\n📌 `/log 1`", parse_mode="Markdown")
        return
    ...
    try:
        ...
        # ====== LẤY THÔNG TIN ======
        uid = getattr(bot, 'botid', 'N/A')
        name = getattr(bot, 'nickname', 'N/A')
        region = getattr(bot, 'region', 'N/A')
        token = getattr(bot, 'token', 'N/A')
        base_url = getattr(bot, 'base_url', 'N/A')
        client_version = getattr(bot, 'client_version', 'N/A')
        ...
        if token != 'N/A':
            try:
                ...
                if exp_timestamp:
                    expiry_time = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(exp_timestamp))
                account_id = decoded.get('account_id', 'N/A')
                external_id = decoded.get('external_id', 'N/A')
                signature = decoded.get('signature_md5', 'N/A')
            except:
                pass
        
        # ====== ONLINE ======
        is_online = bot.running_event.is_set() if bot.running_event else False
        
        # ====== UPTIME ======
        uptime_seconds = 0
        if is_online and hasattr(bot, 'online_time'):
            uptime_seconds = int(time.time() - bot.online_time)
        elif is_online:
            uptime_seconds = 0
        ...
        uptime_str = f"{days} ngày {hours} giờ {minutes} phút {seconds} giây" if days > 0 else f"{hours} giờ {minutes} phút {seconds} giây"
        
        # ====== IP ======
        chat_ip = getattr(bot, 'ChatIP', 'N/A')
        chat_port = getattr(bot, 'ChatPort', 'N/A')
        online_ip = getattr(bot, 'OnlineIP', 'N/A')
        online_port = getattr(bot, 'OnlinePort', 'N/A')
        
        # ====== GUILD ======
        guild_id = getattr(bot, 'GuildIds', 'N/A')
        authen_code = getattr(bot, 'AuthenCode', 'N/A')
        
        # ====== ROOM ======
        room_id = getattr(bot, 'roomid', 'N/A')
        
        # ====== RECONNECT ======
        reconnect_count = getattr(bot, 'reset_count', 0)
        
        # ====== KEY + IV (FULL) ======
        key = getattr(bot, 'key', 'N/A')
        iv = getattr(bot, 'iv', 'N/A')
        if key != 'N/A' and isinstance(key, bytes):
            key = key.hex()
        if iv != 'N/A' and isinstance(iv, bytes):
            iv = iv.hex()
        
        # ====== SỐ BOT ======
        total_bots = len(TCPbot.bots)
        online_bots = sum(1 for b in TCPbot.bots.values() if b.running_event.is_set())
        
        # ====== LOG FILE ======
        log_paths = [
            f"logs/bot{bot_id}.log",
            f"bot{bot_id}.log",
            f"/opt/render/project/src/logs/bot{bot_id}.log",
            f"/opt/render/project/src/bot{bot_id}.log"
        ]
        log_exists = any(os.path.exists(p) for p in log_paths)
        
        users = bot.bot_config.get('access_bot', [])
        user_count = len(users)
        
        token_display = token if token != 'N/A' else 'N/A'
        ...
        if len(json_text) > 4000:
            import io
            file_bytes = io.BytesIO(json_text.encode('utf-8'))
            file_bytes.name = f"bot{bot_id}_log.json"
            telegram_bot.send_document(
                message.chat.id,
                file_bytes,
                caption=f"🤖 Log bot {bot_id}"
            )
        else:
    ...
    try:
        ...
        if len(parts) >= 2:
            ...
            for i, user in enumerate(access_list, 1):
                ...
                try:
                    import requests
                    resp = requests.get(f"http://zantreobot.onrender.com/info1?uid={uid}", timeout=5)
                    if resp.status_code == 200:
                        data = resp.json()
                        if data.get('success'):
    ...
    try:
        ...
        if len(name) > 10:
            telegram_bot.reply_to(
                ...
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"🔴 Tên chỉ được tối đa <b>10</b> ký tự!\n"
                f"📌 Tên hiện tại: <code>{name}</code> ({len(name)} ký tự)\n"
                f"💡 Vui lòng rút gọn tên (tối đa 10 ký tự).</blockquote>",
                parse_mode="HTML"
            )
            return
        ...
        # ====== CHECK LIMIT DAILY ======
        reg_limit = get_user_reg_limit(user_id)
        reg_today = get_user_reg_today(user_id)

        if reg_today >= reg_limit and not is_admin:
            telegram_bot.reply_to(
                message,
                f"<blockquote><b>⛔ ĐÃ ĐẠT GIỚI HẠN</b>\n"
                f"━━━━━━━━━━━━━━━━━━━━\n"
                f"📊 Hôm nay: <code>{reg_today}/{reg_limit}</code>\n"
                f"⏰ Reset lúc: <code>00:00</code>\n"
                f"💡 Liên hệ: @zanbackj để tăng limit</blockquote>",
                parse_mode="HTML"
            )
            return
                ...
                if results:
                    ...
                    for i, acc in enumerate(results, 1):
                        ...
                        if status == 'success':
                            ...
                            file_content += f"    Access Token: {acc.get('access_token')[:30]}...\n"
                            file_content += f"    Open ID: {acc.get('open_id')}\n"
                            file_content += f"    ------------------------------\n"
                            if acc.get('access_token'):
                                tokens.append(acc.get('access_token'))
                        else:
                            file_content += f"[{i}] ❌ {acc.get('name')}: {status}\n"
                            file_content += f"    ------------------------------\n"
                    
                    # Cập nhật số lượng reg
                    update_user_reg(user_id, success_count)
                    ...
                    caption = (
                        ...
                        f"━━━━━━━━━━━━━━━━━━━━\n"
                        f"✅ Thành công: <code>{success_count}/{count}</code> acc\n"
                        f"❌ Thất bại: <code>{count - success_count}</code> acc\n"
                        f"🌍 Region: <code>{region}</code>\n"
                        f"👤 Prefix: <code>{name}</code>\n"
                        f"━━━━━━━━━━━━━━━━━━━━\n"
                        f"📥 File: <code>accounts.txt</code></blockquote>"
                    )
...
def telegram_xkb(message):
    ...
    try:
        ...
        import requests
        url = f" https://zantreobot.onrender.com/xkb?uid={uid}&botid={bot_id}"
...
def telegram_xkb_all(message):
    ...
    try:
        ...
        import requests
        url = f"http://zantreobot.onrender.com/xkb/all?uid={uid}"
...
def keep_alive_external():
    url = "https://zantreobot.onrender.com/check"
    while True:
        try:
            resp = requests.get(url, timeout=10)
            if resp.status_code == 200:
                print(f"[KEEP ALIVE] ✅ Ping thành công - {url}")
            else:
                print(f"[KEEP ALIVE] ⚠️ Status: {resp.status_code}")
        except Exception as e:
...
if __name__ == "__main__":
    threading.Thread(target=keep_alive_external, daemon=True).start()    
