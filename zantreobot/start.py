...
💡 Dùng `/ping` để kiểm tra kết nối"""
        ...
        telegram_bot.reply_to(message, text, parse_mode="Markdown")
        
    except ImportError:
        telegram_bot.reply_to(message, "❌ Thiếu module psutil!\nCài: `pip install psutil`", parse_mode="Markdown")
    except Exception as e:
        telegram_bot.reply_to(message, f"❌ Lỗi: {str(e)[:50]}")
...
def telegram_uptime(message):
    ...
    minutes = (seconds % 3600) // 60
    secs = seconds % 60
    text = f"⏱️ Bot đã chạy: {days} ngày {hours} giờ {minutes} phút {secs} giây"
