 ...
 def C1200(self, data, client):
  try:
   ...
   if is_admin and message.startswith("@"):
    ...
    if 'access_bot' not in bot.bot_config:
        self._bot.reply(cid, type, "User not found")
        return
    ...
    bot.bot_config['access_bot'] = [u for u in bot.bot_config['access_bot'] if u.get('uid') != int(del_uid)]
    self.manager.save_config()
    self._bot.reply(cid, type, f"✅ ĐÃ XÓA QUYỀN UID {del_uid} THÀNH CÔNG!")
    return
    ...
    if parts[1].lower() == "all":
        try:
            # ====== LẤY DANH SÁCH TRỰC TIẾP TỪ playerstatus ======
            ids = []
            
            # Gửi request để cập nhật playerstatus
            self.sock39699.send(self._bot.get_history(self.botid))
            time.sleep(1.5)
            
            # Lấy từ playerstatus (cách 1)
            ...
            # Nếu vẫn rỗng, lấy từ self.ids (cách 2)
            if not ids:
                ids = list(set(self.ids)) if self.ids else []
            
            # Nếu vẫn rỗng, thử gửi lại request lần 2
            if not ids:
...
""")
    ...
    self._bot.reply(cid, type, f"""[E0FFFF]uid: {uid}
[E0FFFF]name: "{name}"
[E0FFFF]telegram: "@zanbackj"
[E0FFFF]time: "{exps}" """)  
    ...
    try:
        ...
        if self.stop_actions.is_set():
            ...
            self._bot.reply(
                cid,
                type,
                f"[B][C][00ff00]✅ Hoàn tất random full súng 7 cho {len(target_uids)} UID\n"
...
[C0C0C0]Bot đã zalo ver 18 thành công và sẽ rời đội ngay!"""
    ...
    if len(parts) < 3: self._bot.reply(cid, type, "[B][c]/msg [teamcode] [message]"); return
    tcode = int(parts[1])
...
def telegram_clear_friends(message):
    ...
    try:
        ...
        try:
            ...
            if response.status_code == 200:
                from zandev.lib import protobuf_dec
                import json
